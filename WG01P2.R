...
## 3. Beta's are assigned to each individual (using rnorm) and stored in 'beta' the bottom 10% is identified and stored
...
##    moving from E -> I in each group is recorded for that data
...
maxxx <- which.max(xx) ## infection peak of whole population
maxyy <- which.max(yy) ## infection peak of cautious 10% population
maxzz <- which.max(zz) ## infection peak of random sample 0.1% population

## Plotting various cases on a common scale after normalization

plot(day, xx, pch = 19, cex = 0.5, main = "New Infection Trajectories - Initial Run" , xlab = "Day number", ylab = "Percentage of cohort newly infected (%)", type = 'l', col = 'brown', ylim = c(0,3.5))
lines(yy, col = 'green')
...
legend("topleft", inset = 0.05, legend = c("Whole population", "Cautious 10% of the popultion", "Random sample of 5,500"), col = c("brown", "green", "cadetblue"), lty = 1:1, cex = 0.7)
...
## of the pandemic is plotted for each sample run alongside a strip chart and box plot. The box plot shows the variability in pandemic peak for the 10 runs,
## however, it ignores the pairwise variability or day on day changes in infection peaks. The line plot shows the infection peak in each of the 10 simulations
## The first strip chart shows the day on day variability by standardizing the infection peak movement from one sample run to the next to 10 days movement and 
## showing the corresponding movement in the other cohorts peak. While the second strip chart shows the change in cohorts infection peak from run to run compared
...
sd1 <- sd2 <- sd3 <- rep(10, 9)  ## initialize vector to store the standardized difference in the infection peak between each run corresponding to 10 day movement for whole population
dd2 <- dd3 <- rep(0, 9)  ## initialize vector to store the  difference in the infection peak movement compared to the whole populations infection peak movement 
...
for (i in 1:9) { ## Loop over each day to the next
  ...
  d2[i] <- v2[i+1] - v2[i] ## Storage of change in infection peak for cautious 10% of population
  d3[i] <- v3[i+1] - v3[i] ## Storage of change in infection peak for sample 0.1% of population
...
dd2 <- d2 - d1 # Difference between move in infection people for whole population compared to cautious 10% population
dd3 <- d3 - d1 # Difference between move in infection people for whole population compared to random 0.1% population
...
## Strip chart representing the movement in infection peaks corresponding to a 10 day move in the infection peak of whole population
...
## Strip chart representing the movement in infection peaks compared to the move in the infection peak of whole population from sample run to sample run

datapeak <- list("Cautious 10%"= dd2, "Random sample" = dd3) ##list of standardized movements in infection peak for three cohorts

stripchart(datapeak, main = "Cohort's Infection peak movement Vs \nmove in population peak ", xlab = "Move in infection peak from run to run compared\n to the infection peak movement in whole population (days)", ylab = "Cohort", method = "jitter", col = c("brown","green", "cadetblue"), pch = 16)


## Conclusion
## The graphs plotted show that infection trajectories simulated using the ZOE data app will have a later peak compared to simulation of the whole population given they are cautious 
